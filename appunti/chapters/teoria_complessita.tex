...
\begin{align*}
    ...
    & s, t \in V \\
    \texttt{soluzioni:} \quad & s=<v_1, \dots, v_k> \in \mathbb{N}^* \\
    i \, \bpi{} s \text{ se } \quad & 
    % \text{dove} \quad &
    v_i \in V \\
    & v_1=s, \: v_k=t \\
    ...
    \texttt{costo:} \quad & c\left( < v_1, \ldots, v_k > \right) = k-1 \\
    & c( \varepsilon ) = \infty \\
    ...
    \texttt{istanza:} \quad & <G=(V,E), s, t, k> \\
...
\begin{align*}
    ...
    \texttt{domanda:} \quad & G \text{ contiene un ciclo semplice che tocca tutti i nodi?} \\
    \text{TSP:} & \quad \\
    \texttt{istanza:} \quad & <G_C=(V,E_C), w, k> \\
    \text{dove} \quad & G_C \text{ grafo completo non orientato} \\
    & w : E \to \mathbb{N} - \left\{ 0 \right\} \\
    & k \in \mathbb{N} \\
    \texttt{domanda:} \quad & G \text{ contiene un ciclo \textit{Hamiltoniano} di costo $\leq k$?} \\
...
A livello macroscopico gli input di $<C_{V_{L}},x>$ sono legati al possibile certificato e l'output è associato al valore del verificatore.
% TODO disegno black box

Il circuito che rappresenta un singolo passo elementare della computazione di un $C_{RAM}$ si dice \textit{gadget}, i cui ingressi e taglia possono essere studiati più a fondo: 
...
Gli ingressi al primo livello del circuito sono valori precisi che rappresentano: il codice del verificatore, il valore di x, la memoria (inizializzata a 0) e i registri (program counter...). Questi input permettono di simulare una memoria, per ogni ingresso corrisponde un filo d'uscita, che indica lo stato della memoria in uscita dopo il passo appena eseguito della macchina RAM. Il \textit{gadget} è il circuito che implementa una singola transizione di stato: in base al valore del PC, estrae ed esegue l'istruzione associata (load, somma...). Tutte queste operazioni si possono implementare con circuiti combinatori.
...
\begin{definition}[Formula]
    \label{def:formula}
    Una formula $<\Phi \left( x_1, \cdots, x_n \right)>$ è un'espressione totalmente parentesizzata, costruita su $\vec{x}$ e gli operatori (connettori):
    $\wedge, \vee, \neg, \to, \leftrightarrow$
\end{definition}
...
\begin{align*}
    SAT:& \\
    \texttt{istanza:} \quad & <\Phi \left( x_1, \cdots, x_n \right)> \\
...
\begin{equation*}
    BC\_SAT \lp SAT
\end{equation*}
Nota: si riduce \textit{dal} noto all'ignoto. Se si sa che il lato sinistro è difficile, e si mostra che è più semplice dell'ignoto, l'ignoto deve essere difficile. L'altro verso è inutile, $BC\_SAT$ è $\bnph$, quindi tutti i problemi $\bnp$ si riducono ad esso.
...
Per esempio un gate $AND$ di indice $g$, con input $x_1^g, \cdots, x_k^g, y_1^g, \cdots, y_h^g$, dove y sono output di altri gate, avrà \textit{fan-in} $k+h$, e si può scrivere la clausola associata che lega il valore di uscita al gate ai valori di ingresso
\begin{equation*}
...
\begin{equation*}
    ...
    \right)
\end{equation*}
La funzione è calcolabile in tempo polinomiale, perché ogni singola clausola ha al massimo un numero di simboli pari agli elementi di circuito incidenti sul singolo gate (\textit{fan-in}), più un filo d'uscita e gli operatori che servono a connettere tra loro gli ingressi, per un totale di al massimo 3 volte gli elementi che incidono sul gate. 
\begin{equation*}
...
Dimostra la NP-completezza di un problema da cui poi si dimostrano molti altri problemi, seguendo una riduzione per \textit{restrizione}: anche se si restringe a una formula con ingressi che sono sottoinsieme più piccolo degli ingressi, resta difficile.
...
\begin{align*}
    3-CNF-SAT: & \\
    \texttt{istanza:} \quad & <\Phi \left( x_1, \cdots, x_n \right)> \text{ in 3-CNF} \\
    \texttt{domanda:} \quad & \Phi \text{ è soddisfacibile?}
\end{align*}
...
\end{equation*}
È necessario un preprocessing del circuito, che, senza perdita di generalità, viene trasformato in un circuito con \textit{fan-in} massimo dei gate pari a 2, che implementa la stessa funzione booleana $C \equiv C'$. Si sfrutta l'associatività delle operazioni $AND$ e $OR$, e si sostituiscono i gate con più di 2 ingressi con una cascata di gate, ciascuno da 2 ingressi. La taglia al massimo triplica, sono necessari infatti $k-1$ gate e fili di uscita aggiuntivi $|<C'>| \le 3 \, |<C>|$.
...
\begin{align*}
    ...
    y_o & \equiv ESERCIZIO
\end{align*}
Occorre un ulteriore passaggio ``cosmetico'' per far comparire \textit{tre} letterali distinti in disgiunzione, aumentando artificialmente il numero di variabili booleane in modo che
\begin{equation*}
...
\begin{equation*}
    | \Phi \left( \vec{x}, \vec{y}, \vec{z} \right) | = \Theta \left( |<C>| \right)
\end{equation*}
...
Per dimostrare che $CLIQUE \in \bnph$ si riduce da $3-CNF-SAT$ a $CLIQUE$ (sempre \textit{dal} noto difficile all'ignoto), cercando una riduzione dalle formule $3-CNF$ ad un grafo.
\begin{equation*}
    3-CNF-SAT \lp CLIQUE
\end{equation*}
...
Negativa: quando due letterali \textit{non} possono essere contemporaneamente veri sotto lo stesso assegnamento di verità? Solo nel caso in cui uno è la negazione dell'altro ($x_h, \overline{x_h}$).
...
\begin{equation*}
    ...
    \Leftrightarrow
    f \left( \Phi \right) = 
    < G_{\Phi} = \left( V_{\Phi} , E_{\Phi} \right), \, k_{\Phi} >
    \: \in CLIQUE
\end{equation*}
\begin{proof}
    ``$\Rightarrow$''
    ...
    Si supponga per assurdo che non sia una clique, allora
    $
        y_{i_1}^{\,j_{i_1}}
        = \neg \,
        y_{i_2}^{\,j_{i_2}}
