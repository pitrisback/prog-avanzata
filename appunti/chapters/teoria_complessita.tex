...
Informalmente, dato un programma $f$ che decide se le computazioni terminano, si può sempre trovare un programma patologico $g$ che, passato a $f$ insieme ad un input $x$, fa poi per quell'input l'opposto di quello che $f$ predice.
% \begin{equation*}
    % g(x) \ne f(g,x)
% \end{equation*}
...
Informalmente, c'è forte evidenza che il \emph{lower bound} esista, infatti si può provare che tutti i problemi in questa classe sono equivalenti, e questa proprietà di chiusura spinge verso la loro difficoltà.
...
Sotto l'ipotesi $\bp{} \neq \bnp{} $, si trova una classe di linguaggi $\bnpc{} \subseteq \bnp{} - \bp{} $ detti NP-completi. Si può costruire una relazione d'ordine tra linguaggi, e provare che gli $\bnpc{}$ sono i $\sup$ di questa relazione, e sono tutti equivalenti. Se uno di questi appartenesse a $\bp{}$, l'intera classe collasserebbe in $\bp{}$.
...
\begin{figure}[H]
    \centering
    \caption{Grafo complemetare}
    \label{fig:grafocomp}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \caption{Grafo originale}
        \label{fig:grafocomp:orig}
        \begin{tikzpicture} [
                scale=1,
                vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \caption{Grafo complementare}
        \label{fig:grafocomp:comp}
        \begin{tikzpicture} [
                scale=1,
                vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
                edge/.style = {draw,thick,-},
...
I due problemi sono complementari:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture} [
            scale=1,
            vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
            is vertex/.style = {vertex, fill=blue!24},
            vc vertex/.style = {vertex, fill=red!24},
            edge/.style = {draw,thick,-},
        ]
        \foreach \pos/\name in {
        {(0,1.8)/c}, {(2.5,0)/d}, {(0.5,0)/b}, {(3,1.8)/e}, {(1.5,3)/a}}
            \node[vertex] (\name) at \pos {$\name$};
        \foreach \source/ \dest in {
                c/e,b/e,b/d,a/e,d/e,c/d}
            \path[edge] (\source) edge (\dest);
        \foreach \vertex in {a,b,c}
            \path node[is vertex] at (\vertex) {$\vertex$};
        \foreach \vertex in {d,e}
            \path node[vc vertex] at (\vertex) {$\vertex$};
    \end{tikzpicture}
\end{figure}
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}[b]{0.45\textwidth}
            ...
            \begin{tikzpicture} [
                ...
                \foreach \source/ \dest in {
                        d/x1}
                    \path[dotted edge] (\source) edge (\dest);
            \end{tikzpicture}
        \end{subfigure}
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \begin{subfigure}[b]{0.45\textwidth}
            \centering
            \caption{Adiacenza replicata per entrambi i nodi}
            \label{fig:lopaham:comp}
            \begin{tikzpicture} [
                    scale=1,
                    vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
                    selected vertex/.style={circle,fill=blue!25,minimum size=20pt,inner sep=0pt},
...
\begin{figure}[H]
    \centering
    \caption{Creazione dei triangoli con apice $u_e$}
    \label{fig:dosevc}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \caption{Grafo originale}
        \label{fig:dosevc:orig}
        \begin{tikzpicture} [
                auto,
                scale=1,
                vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
            ...
            \foreach \source/ \dest /\weight in {
                    d/c/e_1,d/a/e_2,a/b/e_3,b/d/e_4}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \caption{Grafo espanso}
        \label{fig:dosevc:triangoli}
        \begin{tikzpicture} [
                auto,
                scale=1,
                vertex/.style={circle,fill=black!25,minimum size=20pt,inner sep=0pt},
            ...
            \foreach \pos/\name in {
                    ...
                    {(2,1)/d}}
                \node[vertex] (\name) at \pos {$\name$};
            % nuovi vertici colorati, con label
            \foreach \pos/\name/\label in {
                    {(0,0)/e2/$u_{e_1}$},
                    {(0,4)/e3/$u_{e_2}$},
                    {(4,2)/e4/$u_{e_3}$},
