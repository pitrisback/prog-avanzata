...
altri inressi che al primo livello del circuito sono valori precisi che sono: il codice del verificatore, lo spazio dove si trova x; altri ingressi che sono memoria (a 0) e i registri (program counter...). questi input permettono di mantenere memoria, per ogni ingresso avro un filo d'uscita, che indica la memoria in uscita dopo un certo passo della macchina ram. è il circuito che implementa una singola transizione di stato: guarda il PC, vede che istruzione è associata e la esegue (load, somma...) tutte operazioni che si possoon implementare con circuiti combinatori.
