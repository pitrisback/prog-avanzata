...
\subsection{Problema della \textit{boolean circuit satisfiability} (BC\_SAT)}
...
L'idea della riduzione di \textit{Cook} è di costruire, per ogni verificatore $V_L$ e istanza (stringa) $x$, un circuito $C_{V_L,x}$ (ovvero un'istanza di $BC\_SAT$), tale che
\begin{itemize}
    ...
    \item se $x \notin L$ (la stringa è un'istanza negativa del linguaggio), non esiste nessun certificato $y_x$ per cui il circuito segnerà in output $1$ e il circuito sarà \textit{non} soddisfacibile
\end{itemize}
Formalmente:
\begin{equation*}
...
\begin{equation*}
    f_l : x \in \setzo{*} \to f(x) = <C_{V_{L}},x>
\end{equation*}
tale per cui
\begin{equation*}
...
Il circuito che rappresenta un singolo passo elementare di un $C_{RAM}$ si dice \textit{gadget}, la cui taglia può essere studiata più a fondo: 
...
cvlx a livello macroscopico gli input sono legati al possibile certificato e l'output e associato al valore associato al verificatore.

TODO disegno black box

il circuito è associato al sottocircuito, detto gadget, che simula il singolo passo di computazione.

altri ingressi che al primo livello del circuito sono valori precisi che sono: il codice del verificatore, lo spazio dove si trova x; altri ingressi che sono memoria (a 0) e i registri (program counter...). questi input permettono di mantenere memoria, per ogni ingresso avro un filo d'uscita, che indica la memoria in uscita dopo un certo passo della macchina ram. è il circuito che implementa una singola transizione di stato: guarda il PC, vede che istruzione è associata e la esegue (load, somma...) tutte operazioni che si possono implementare con circuiti combinatori.

se il circuito fosse troppo grande rispetto all'ingresso, la funzione non potrebbe essere ctp, dovrebbe generare la circuiteria di taglia esponenziale. va mostrato che il circuito si può realizzare in uno spazio polinomiale in x.

ingressi:

certificato limitato a lunghezza max di certificato accettabile k1

memoria di cui può fare uso non può essere piu grande del numero di passi che esegue il verificatore, può toccare al più una cella di memoria per passo.

numero di passi che esegue il verificatore: tempo associato e' polinomiale in x+y con k2, la memoria non più di questa quantita, e lo stesso per i registri.
...
un circuito che realizza una computazione su questo numero di input ha al più taglia polinomiale, pensi ad addizionatore o moltiplicatore
...
per emulare una computazione, si costruisce una cascata di gadget, e il valore associato a return VL, seleziono quello come bit in memoria che è output dell'intero circuito

il numero di gadget deve essere sufficiente per eseguire la computazione su x e y che al più sono
\begin{equation*}
    O \left( \left( |x|+|x|^{k_1} \right)^{k_2} \right)
\end{equation*}

e questo si applica ad ogni linguaggio in np, appena ho un verificatore questo mi implica un verificatore, e esiste un circuito associato. i dettagli ora sono solo la circuiteria del modello ram.
...
questa catena vale da dx a sx e viceversa, e quindi prova che
...
\begin{definition}
    una formula $<\Phi \left( x_1, \cdots, x_n \right)>$ è un'espressione totalmente parentesizzata, costruita su $\vec{x}$ e gli operatori (connettori) seguenti:
    $\wedge, \vee, \neg, \to, \leftrightarrow$
    \label{def:formula}
\end{definition}
...
L'equivalenza si riscrive come a
...
l'albero è la codifica della formula, posso scriverla come albero e l'albero lo codifico come grafo. quando si parla di codifiche ragionevoli, si finisce sempre alla codifica di insieme di interi, e le codifiche ragionevoli si scrivono a partire dagli interi . . .

si definisce assegnamento di valori di verita alle variabili di una formula per sat un vettore di valori 0 1, come bcsat, sotto un assegnamento di valori di verita la formula ha un valore vero o falso

si dice soddisfacibile se esiste b per cui vale 1

\subsection{Da BC\_SAT a \textit{formula satisfiability} (SAT)}

sat:

istanza: $<\Phi \left( x_1, \cdots, x_n \right)>$

domanda: $\Phi$ è soddisfacibile?

sat appartiene a NPC va provato

sat in NP semplice da mostrare, scrivi verificatore $V_{SAT}(x,y)$ per SAT come ESERCIZIO, risali l'albero e trovi il valore alla radice. computazione lineare nella lunghezza della formula

vogliamo mostrare che sat è NP-hard, usando la transitività, prendo un problema noto in NPC o NPH e provare che questo problema noto si riduce al candidato SAT. ossia
\begin{equation*}
    BC\_SAT \lp SAT
\end{equation*}

NOTA: si riduce DAL noto all'ignoto.

so che sx è difficile, e mostro che è più semplice dell'ignoto, l'ignoto deve essere difficile.

(l'altro verso è inutile, bcsat è NPH, quindi tutti i problemi NP si riducono ad esso)
...
usando quattro gate per modulo, implemento 
$
o_2 = \left( x_1 \wedge \neg \, x_2 \right)
o_1 = \left( \neg \, x_1 \wedge x_2 \right)
$

in un modulo generico $i$
...
sia $S(i)$ la taglia della funzione al passo $i$, si nota che al passo i contiene due funzioni del passo i-1 più altri simboli, per cui vale $S(i) \ge 2 S (i-1)$, in più vale $S(1)=c$ costante, e si crea una minorazione per ricorrenza che ha come soluzione $S(i) \ge c \cdot 2^i$, di conseguenzza dopo $n$ stati vale $S(n) \ge c \cdot 2^n$
\begin{equation*}
    |< \Phi \left( x_1, x_2 \right) >|
    = \Omega \left( 2^{n} \right)
\end{equation*}

da una codifica $\Theta (n)$ si arriva ad una complessità esponenziale

\subsubsection{Scrittura efficiente della formula}

il circuito si può vedere come grafo
$c \equiv \left( I \cup G \cup \left\{ o \right\}, W \right)$
associo le variabili $x$ agli input, e variabili addizionali $y_g$, una per ogni gate, che indica il valore di uscita ad un gate sotto una configurazione d'ingresso. Simuliamo la commutazione del circuito con una serie di equazione logiche dette clausole.
Per esempio un gate AND di indice g con input $x_1^g, \cdots, x_k^g, y_1^g, \cdots, y_h^g$, dove y sono output di altri gate, avra fan in k+h, e si può scrivere la clausola associata che lega il valore di uscita al gate ai valori di ingresso
\begin{equation*}
...
\right)
\end{equation*}
E si avrà una clausola per ogni gate. a questo punto si può definire una prima formula detta $Conf$ perche indica la configurazione di un circuito sotto un certo ingresso:
\begin{equation*}
    Conf \left( \vec{x}, \vec{y} \right) =
    \bigwedge\limits_{g \in G}
    C_g \left( \vec{x}, \vec{y} \right)
\end{equation*}
Si prova per riduzione che vale il seguente teorema:
\begin{theorem}
    $ Conf ( \vec{b_1}, \vec{b_2} ) = 1 \Leftrightarrow $ sotto $\vec{b_1}$, $C( \vec{b_1} )$ commuta e ogni gate in uscita ha valore $( \vec{b_2} )_g$
    \label{teo:commuta}
\end{theorem}
...
si vuole invece scrivere una formula che sia soddisfacibile solo se il circuito originale lo è, usando Conf come gadget.

notiamo che esiste un gate particolare $g_o$ legato al filo d'uscita $y_{g_o}$
...
\begin{equation*}
    ...
    \right)
\end{equation*}
f è ctp perché ogni singola clausola ha simboli max pari agli elementi di circuito incidenti sul singolo gate, fan in più filo d'uscita più operatori che servono a connettere tra loro gli ingressi, al massimo 3 volte gli elementi che incidono sul gate. 
\begin{equation*}
    | \Phi \left( \vec{x}, \vec{y} \right) | = \Theta \left( |<C>| \right)
\end{equation*}

ora mostrato

\begin{equation*}
    < C \left( x_1, \cdots, x_n \right) > \in BC\_SAT
    \Leftrightarrow
    < \Phi \left( \vec{x}, \vec{y} \right) > \in SAT
\end{equation*}
...
\begin{proof}
    ...
    Dunque si ha che $Conf ( \vec{b_1}, \vec{b_2} ) \wedge y_{g_o} = 1 \Rightarrow ( \vec{b_1}, \vec{b_2} ) $ è un assegnamento che soddisfa $\Phi = f (<C>)$

    ``$\Leftarrow$''
    $f(<C>) \in SAT \Rightarrow \exists \vec{b_1}, \vec{b_2} : \Phi ( \vec{b_1}, \vec{b_2} ) = 1$.
...
Dimostra la NP completezza di un problema da cui poi si dimostrano molti altri problemi.

Con una riduzione per \textit{restrizione}: anche se si restringe a una formula con ingressi che sono sottoinsieme più piccolo degli ingressi, resta difficile.

Spesso è interessante restringere finche diventa facile.
...
istanza: $<\Phi \left( x_1, \cdots, x_n \right)>$ in 3-CNF
