...
\begin{definition}
    ...
    Dati, $a,b \in zsn$, il logaritmo discreto è $k$ tale che
    \begin{equation*}
        a^k \equiv b mon n
    \end{equation*}
\end{definition}

Questo problema sembra avere lo stesso status della fattorizzazione, è NP ma non è NP-Hard.
...
\subsubsection{Messaggi non in zetastar???}

Un qualsiasi messaggio non in zetastar cracca il sistema.
Infatti, se
$M \notin \zs{n}$
allora deve valere $
gcd(M, n) > 1
...
gcd(M, n) = q
$. 
Questo valore si può ricavare con Euclid, ed è stato fattorizzato $n$.
